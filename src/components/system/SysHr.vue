<template>
  <div style="margin-top: 10px" v-loading="fullloading">
    ...
    <div style="display: flex;justify-content: space-around;flex-wrap: wrap;text-align: left">
      <el-card style="width: 350px;margin-bottom: 20px" v-for="(item,index) in userList" :key="item.id"
        ...
        <div>
          ...
          <div style="margin-top: 20px">
            ...
            <div><span class="user-info">电话号码:{{item.phone}}</span></div>
            <div class="user-info" style="display: flex;align-items: center;margin-bottom: 3px">
              用户状态:
              <el-switch
                style="display: inline;margin-left: 5px"
                v-model="item.enabled === 1"
                active-color="#13ce66"
                inactive-color="#aaaaaa"
                active-text="启用"
                :key="item.id"
                @change="switchChange(1-item.enabled,item.id,index)"
                inactive-text="禁用">
              </el-switch>
            </div>
            <div class="user-info">
              ...
              <el-tag
                ...
                :disable-transitions="false">{{role.description}}
              </el-tag>
              <el-popover
  ...
  export default{
    ...
    methods: {
      ...
      updateHrRoles(hrId, index){
        ...
        this.putRequest("/permission/updateUserRoles", {
          userId: hrId,
          rids: this.selRoles
        }).then(resp=> {
      ...
      refreshHr(hrId, index){
        ...
        this.getRequest("/permission/getUserById?id=" + hrId).then(resp=> {
          _this.cardLoading.splice(index, 1, false)
          _this.userList.splice(index, 1, resp.data.data);
        })
      },
      ...
      loadAllRoles(){
        var _this = this;
        this.getRequest("/permission/roles").then(resp=> {
      ...
      switchChange(newValue, hrId, index){
        var _this = this;
        _this.cardLoading.splice(index, 1, true)
        this.putRequest("/permission/updateUser", {
          enabled: newValue,
          id: hrId
        }).then(resp=> {
          _this.cardLoading.splice(index, 1, false)
      ...
      initCards(){
        ...
        this.getRequest("/permission/userSearchByKeywords?keywords=" + searchWords).then(resp=> {
          if (resp && resp.status === 200 && resp.data.errCode === 0) {
