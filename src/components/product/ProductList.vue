...
<script>
  export default {
    ...
    methods: {
      userListCriteria() {
        let _this = this;
        console.log(JSON.stringify(_this.criteria))
      ...
      tabClick: function (tab, event) {
        ...
        switch (event.target.getAttribute('id')) {
          ...
          case 'tab-2': {
            this.criteria.state = 1;
            this.productTotalStat();
            this.productStatCriteria();
            break
          }
          default:
            break
      ...
      userEnabledEdit: function (index, row) {
        this.$confirm('确定下架 ' + row.name + ' 吗？', '产品下架提示', {
          distinguishCancelAndClose: true,
          confirmButtonText: '确定',
          cancelButtonText: '取消'
        }).then(() => {
          $.post("/manage/product/putOff/" + row.id, function (data) {
            app.$message({
              type: 'success',
              message: '下架成功'
            });
            app.userListCriteria();
          });
        }).catch(action => {
          this.$message({
            type: 'info',
      ...
      onSale: function (index, row) {
        this.$confirm('确定上架 ' + row.name + ' 吗？', '产品上架提示', {
          distinguishCancelAndClose: true,
          confirmButtonText: '确定',
          cancelButtonText: '取消'
        }).then(() => {
          $.post("/manage/product/putOn/" + row.id, function (data) {
            app.$message({
              type: 'success',
              message: '下架成功'
            });
      ...
      userEdit: function (productId) {
        window.location.href = '/manage/shareProduct/edit?productId=' + productId;
      },
      productDetail: function (product) {
        window.location.href = '/manage/shareProduct/detail?productId=' + product.id + '&productName=' + product.name;
      },
      productDetail2: function (product) {
        window.location.href = '/manage/shareProduct/detail?productId=' + product.productId + '&productName=' + product.productName;
      }
    }
  }
</script>
